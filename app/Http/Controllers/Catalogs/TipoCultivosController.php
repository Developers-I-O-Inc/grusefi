...
class TipoCultivosController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($tipo_cultivos)
                    ->addIndexColumn()
                    ...
                    ->addColumn('buttons', function($row){
                        $btn = '<button data-id="'.$row->id.'" type="button" class="btn btn-active-light-success btn-sm me-0 ms-0" data-kt-tipo_cultivo-table-filter="edit" data-bs-toggle="tooltip" title="Editar">
        ...
        return view('catalogs/tipo_cultivos');
    ...
    {
        $tipo_cultivo=TipoCultivos::find($id);

        return response()->json(['tipo_cultivo'=>$tipo_cultivo]);
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy_tipo_cultivos(Request $request)
    {
        $ids = $request->input('ids');
        TipoCultivos::whereIn('id', $ids)->delete();
