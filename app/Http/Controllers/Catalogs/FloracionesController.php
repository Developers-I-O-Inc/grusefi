...
class FloracionesController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $floraciones = Floraciones::all();
            return Datatables::of($floraciones)
                    ->addIndexColumn()
                    ...
                    ->addColumn('buttons', function($row){
                           $btn = '<button data-id="'.$row->id.'" type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-kt-floracion-table-filter="edit">
        ...
        return view('catalogs/floraciones');
    ...
    {
        Floraciones::updateOrCreate(
            ['id'=>$request->get('id_floracion')],
    ...
    {
        $floracion=Floraciones::find($id);

        return response()->json(['floracion'=>$floracion]);
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy_floraciones(Request $request)
    {
        $ids = $request->input('ids');
        Floraciones::whereIn('id', $ids)->delete();
